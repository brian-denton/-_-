...
export function BrutalistHero() {
	...
	const [chaosElements, setChaosElements] = useState<Array<{id: number, x: number, y: number, color: string, size: number, rotation: number}>>([]);
		...
		if (chaosMode) {
			const interval = setInterval(() => {
				...
				const newElements = Array.from({ length: 20 }, (_, i) => ({
					...
					x: Math.random() * 100,
					y: Math.random() * 100,
					color: ['#ef4444', '#eab308', '#3b82f6', '#22c55e', '#a855f7', '#f97316'][Math.floor(Math.random() * 6)],
	...
	return (
		<section className="min-h-screen flex items-center justify-center relative overflow-hidden bg-black">
			...
			<div className={`absolute inset-0 transition-opacity duration-300 ${chaosMode ? 'opacity-30' : 'opacity-10'}`}>
			...
			<div className="absolute inset-0 pointer-events-none">
				<div className={`absolute top-20 left-10 w-32 h-32 bg-red-500 transform rotate-45 opacity-80 transition-all duration-300 ${chaosMode ? 'animate-spin scale-150' : ''}`} />
				<div className={`absolute top-40 right-20 w-24 h-24 bg-yellow-400 opacity-70 transition-all duration-300 ${chaosMode ? 'animate-bounce scale-125' : ''}`} />
				<div className={`absolute bottom-32 left-1/4 w-40 h-20 bg-blue-500 transform -rotate-12 opacity-60 transition-all duration-300 ${chaosMode ? 'animate-pulse scale-110' : ''}`} />
				<div className={`absolute bottom-20 right-10 w-28 h-28 bg-green-500 rounded-full opacity-50 transition-all duration-300 ${chaosMode ? 'animate-ping scale-200' : ''}`} />
			</div>
			...
			<div className={`relative z-10 text-center px-4 max-w-6xl mx-auto transition-all duration-300 ${chaosMode ? 'animate-pulse' : ''}`}>
				{/* Main brutal heading */}
				<div className="mb-8">
					{isLoading ? (
				...
				{!isLoading && content && (
					<div className={`bg-white text-black p-8 border-8 border-black transform rotate-1 hover:rotate-0 transition-transform duration-300 mb-8 max-w-4xl mx-auto ${
						chaosMode ? 'animate-pulse bg-red-500 text-white border-yellow-400 rotate-12' : ''
					}`}>
						<p className="text-xl md:text-2xl font-bold font-mono leading-tight">
							{chaosMode ? "CHAOS MODE ACTIVATED! REALITY IS BREAKING DOWN! EMBRACE THE MADNESS!" : content.heroDescription}
						</p>
					</div>
				)}
			...
			<div className={`absolute top-0 left-0 w-0 h-0 border-l-[100px] border-l-transparent border-b-[100px] border-b-red-500 transition-all duration-300 ${chaosMode ? 'animate-spin' : ''}`} />
			<div className={`absolute top-0 right-0 w-0 h-0 border-r-[100px] border-r-transparent border-b-[100px] border-b-blue-500 transition-all duration-300 ${chaosMode ? 'animate-bounce' : ''}`} />
			<div className={`absolute bottom-0 left-0 w-0 h-0 border-l-[100px] border-l-transparent border-t-[100px] border-t-yellow-400 transition-all duration-300 ${chaosMode ? 'animate-pulse' : ''}`} />
			<div className={`absolute bottom-0 right-0 w-0 h-0 border-r-[100px] border-r-transparent border-t-[100px] border-t-green-500 transition-all duration-300 ${chaosMode ? 'animate-ping' : ''}`} />
		</section>
	);
}