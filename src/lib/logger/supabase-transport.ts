...
import { db, logsTable, InsertLog, LogLevel } from '../db';
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      const validatedLogs = logsToFlush.map((log, index) => {
        try {
          ...
          if (!log.level || !log.message) {
            throw new Error(`Log ${index}: Missing required fields (level: ${log.level}, message: ${log.message})`);
          }
          ...
          console.error(`Log validation failed for record ${index}:`, error, log);
          throw error;
        }
      });
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ Successfully wrote ${validatedLogs.length} logs to database`);
      }
    } catch (error) {
      ...
      this.emit('error', new Error(`Failed to write logs to database: ${errorMessage}`));
    }
  }
 ...
 */
export function createSupabaseTransport(options: SupabaseTransportOptions = {}): SupabaseTransport {
